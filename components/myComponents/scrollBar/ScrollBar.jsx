...
const ScrollBar = () => {
    ...
    return (
        <ul className='min-w-16 fixed right-1 top-1/2 -translate-y-1/2 z-50 flex-col items-center gap-10 hidden xl:flex'>
            ...
            <li tabIndex={0} onFocus={() => setActive('skills')} onBlur={() => setActive('')} onMouseEnter={() => setHover2(true)} onMouseLeave={() => setHover2(false)} className='relative flex justify-center items-center gap-4'>
                <a href="#skills" className={(hover2 || focus2) ? 'main-col absolute right-full mr-1 whitespace-nowrap transition-opacity duration-300 opacity-100 text-xs text-center mb-0.5 pointer-events-auto' : 'soft-col absolute right-full mr-1 whitespace-nowrap transition-opacity duration-300 opacity-0 pointer-events-none'}>Skills</a>
                <a href="#skills" className={'h-0.5 main-bg rounded-full inline-block transition-all duration-300 ' + (focus2 ? 'w-5 opacity-100' : 'w-0 opacity-0')}></a>
                <a href="#skills"><Circle className={'w-1 h-1 soft-bg rounded-full soft-col transition-all duration-300 ' + (focus2 ? 'opacity-0 scale-75' : 'opacity-100 scale-100')} /></a>
            </li>
            <li tabIndex={0} onFocus={() => setActive('gallary')} onBlur={() => setActive('')} onMouseEnter={() => setHover3(true)} onMouseLeave={() => setHover3(false)} className='relative flex justify-center items-center gap-4'>
                <a href="#gallary" className={(hover3 || focus3) ? 'main-col absolute right-full mr-1 whitespace-nowrap transition-opacity duration-300 opacity-100 text-xs text-center mb-0.5 pointer-events-auto' : 'soft-col absolute right-full mr-1 whitespace-nowrap transition-opacity duration-300 opacity-0 pointer-events-none'}>Gallary</a>
                <a href="#gallary" className={'h-0.5 main-bg rounded-full inline-block transition-all duration-300 ' + (focus3 ? 'w-5 opacity-100' : 'w-0 opacity-0')}></a>
                <a href="#gallary"><Circle className={'w-1 h-1 soft-bg rounded-full soft-col transition-all duration-300 ' + (focus3 ? 'opacity-0 scale-75' : 'opacity-100 scale-100')} /></a>
            </li>
